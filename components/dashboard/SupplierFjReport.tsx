...
import { Sale, StockItem, StaffRole, ValeEntrada } from '../../types';
import { useStore } from '../../store/GlobalContext';
import { ValeEntradaPrintModal } from '../stock/ValeEntradaPrintModal';
...
export const SupplierFjReport: React.FC<SupplierFjReportProps> = ({
  ...
  const { valesEntrada } = useStore();

  // Navigation & View state
  const [activeTab, setActiveTab] = useState<'resumen' | 'fardos' | 'ventas' | 'ingresos'>('resumen');
  ...
  // --- FILTRADO ESTRICTO DE PROVEEDOR FJ (EXCLUYENDO PD / POR DEFINIR) ---
  const isFjSupplier = (prov?: string): boolean => {
    if (!prov) return false;
    const p = prov.trim().toUpperCase();

    // EXCLUSIÓN ABSOLUTA de fardos "PD" (Por Definir)
    if (
      p === 'PD' ||
      p === 'POR DEFINIR' ||
      p.startsWith('PD ') ||
    ...
    // Exclusión de otros proveedores conocidos que no son FJ
    const nonFjKnown = ['CANADA', 'IM', 'BETA', 'USA', 'JK', 'FE', 'TOM Y JERRY', 'OLD NAVY', 'ZT', 'RT', 'TIGRE', 'GENERAL'];
    if (nonFjKnown.includes(p)) {
      return false;
    }

    // Coincidencia positiva estricta para FJ
    return p === 'FJ' || p === 'PROVEEDOR FJ' || p.startsWith('FJ ') || p.startsWith('FJ-') || p.startsWith('FJ/');
  };

  // Vales de entrada cuyo ingreso es de FJ
  const fjVales = useMemo(() => {
    return (valesEntrada || []).filter(v => isFjSupplier(v.proveedor) || (v.items || []).some(it => isFjSupplier(it.proveedor)));
  }, [valesEntrada]);

  // Conjunto de códigos ingresados por Vales de Entrada de FJ
  const fjValeItemCodes = useMemo(() => {
    const codes = new Set<string>();
    (valesEntrada || []).forEach(v => {
      const isValeFj = isFjSupplier(v.proveedor);
      (v.items || []).forEach(it => {
        const itProv = (it.proveedor || '').trim().toUpperCase();
        // Si el artículo individual es explícitamente PD o de otro proveedor, se ignora
        if (itProv === 'PD' || itProv === 'POR DEFINIR' || itProv.includes('DEFINIR')) return;
        if (isValeFj || isFjSupplier(it.proveedor)) {
          if (it.codigo) codes.add(it.codigo.trim().toUpperCase());
        }
      });
    });
    return codes;
  ...
  // Mapa de relación código -> Vale de Entrada para trazabilidad
  const itemFjValeMap = useMemo(() => {
    const map = new Map<string, ValeEntrada>();
    (valesEntrada || []).forEach(v => {
      const isValeFj = isFjSupplier(v.proveedor);
      (v.items || []).forEach(it => {
        const itProv = (it.proveedor || '').trim().toUpperCase();
        if (itProv === 'PD' || itProv === 'POR DEFINIR' || itProv.includes('DEFINIR')) return;
        if (isValeFj || isFjSupplier(it.proveedor)) {
          if (it.codigo) map.set(it.codigo.trim().toUpperCase(), v);
        }
      });
    });
    return map;
  ...
  // --- IDENTIFICACIÓN DE FARDOS CUYO INGRESO ES ESTRICTAMENTE DE FJ ---
  const fjStockItems = useMemo(() => {
    return stock.filter(item => {
      const prov = (item.proveedor || '').trim().toUpperCase();
      const code = (item.codigo || '').trim().toUpperCase();

      // 1. REGLA FUNDAMENTAL: Exclusión estricta de fardos "PD" (Por Definir)
      if (
        prov === 'PD' ||
        prov === 'POR DEFINIR' ||
        prov.startsWith('PD ') ||
        prov.startsWith('PD-') ||
        prov.startsWith('PD/') ||
        prov.includes('DEFINIR')
      ) {
        return false;
      }

      // 2. Coincidencia directa por proveedor registrado en el ítem de Stock
      if (isFjSupplier(prov)) {
        return true;
      }

      // 3. Coincidencia si el fardo ingresó mediante un Vale de Entrada de FJ
      if (code && fjValeItemCodes.has(code)) {
        const nonFjKnown = ['CANADA', 'IM', 'BETA', 'USA', 'JK', 'FE', 'TOM Y JERRY', 'OLD NAVY', 'ZT', 'RT', 'TIGRE'];
        if (nonFjKnown.includes(prov)) return false;
        return true;
      }

      return false;
    });
  }, [stock, fjValeItemCodes]);
  ...
  return (
    <div id="supplier-fj-dashboard-section" className="bg-white rounded-[40px] border-2 border-emerald-100 shadow-xl overflow-hidden relative transition-all">
        ...
        <div className="p-6 md:p-8 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
            <div className="flex items-center gap-2 bg-slate-100 p-1.5 rounded-2xl">
              ...
              >
                <Truck size={15} className={activeTab === 'ingresos' ? 'text-emerald-600' : ''} />
                Ingresos / Vales FJ ({fjVales.length})
              </button>
            </div>
                      ...
                      <tr key={f.codigo} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td className="py-3 px-4">
                          ...
                          <div className="flex flex-wrap items-center gap-1.5 mt-1">
                            ...
                            {itemFjValeMap.get(f.codigo.trim().toUpperCase()) && (
                              <span className="px-2 py-0.5 bg-indigo-50 text-indigo-700 border border-indigo-200 rounded text-[9px] font-black uppercase">
                                Vale {itemFjValeMap.get(f.codigo.trim().toUpperCase())?.folio}
                                {itemFjValeMap.get(f.codigo.trim().toUpperCase())?.numeroContenedor ? ` • Cont. ${itemFjValeMap.get(f.codigo.trim().toUpperCase())?.numeroContenedor}` : ''}
                              </span>
                            )}
                            <span className="text-[9px] text-slate-400 font-bold uppercase">
                              • {f.ordenesCount} órdenes
                            </span>
                          </div>
                        </td>
          ...
          {/* TAB 4: HISTORIAL DE INGRESOS Y VALES DE ENTRADA FJ */}
          {activeTab === 'ingresos' && (
            <div className="space-y-4 animate-in fade-in duration-300">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-200/80">
                <div>
                  <h4 className="text-xs font-black uppercase text-slate-900 tracking-wider flex items-center gap-2">
                    <Truck size={15} className="text-emerald-600" /> Vales de Entrada e Ingresos Contenedor FJ
                  </h4>
                  <p className="text-[10px] text-slate-500 font-medium mt-0.5">
                    Historial de comprobantes de ingreso y recepción de mercancía correspondientes al proveedor FJ
                  </p>
                </div>
                <span className="px-3 py-1 bg-emerald-100 text-emerald-800 rounded-full font-black text-xs font-mono self-start sm:self-auto">
                  {fjVales.length} Vales Registrados
                </span>
              </div>
              ...
              {fjVales.length === 0 ? (
                <div className="p-12 text-center bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                  ...
                  <h4 className="text-sm font-black text-slate-800 uppercase">Sin Vales de Entrada Registrados para FJ</h4>
                  <p className="text-slate-400 text-xs mt-1 font-medium max-w-md mx-auto">
                    Los {fjStockItems.length} fardos incluidos en este informe corresponden a los productos catalogados con proveedor "FJ" en el inventario. Al registrar nuevos vales de entrada para FJ, se reflejarán automáticamente aquí con trazabilidad completa.
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                  {fjVales.map((vale) => (
                    <div 
                      key={vale.id}
                      className="bg-white p-5 rounded-2xl border border-slate-200 hover:border-slate-300 transition-all shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4"
                    >
                      <div className="flex items-start gap-3">
                        <div className="w-10 h-10 rounded-xl bg-emerald-600 text-white flex items-center justify-center font-black text-xs shadow-xs shrink-0">
                          <FileText size={18} />
                        </div>
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {vale.numeroContenedor && (
                              <span className="px-2 py-0.5 bg-blue-50 text-blue-700 border border-blue-200 rounded text-[9px] font-black uppercase">
                                Contenedor: {vale.numeroContenedor}
                              </span>
                            )}
                            <span className="px-2 py-0.5 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded text-[9px] font-black uppercase">
                              Proveedor: {vale.proveedor || 'FJ'}
                            </span>
                          </div>
                          <p className="text-xs font-bold text-slate-700 mt-1">
                            {vale.descripcion || 'Ingreso de fardos proveedor FJ'}
                          </p>
                          <p className="text-[10px] text-slate-400 font-bold uppercase mt-0.5">
                            {vale.items?.length || 0} artículos • {vale.totalUnidades || 0} unidades totales • Registrado por: {vale.usuarioRegistro || 'Bodega'}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 self-end md:self-auto">
                        <button
                          type="button"
                          onClick={() => setSelectedValeToPrint(vale)}
                          className="px-4 py-2 bg-slate-900 text-white hover:bg-black rounded-xl font-black text-xs uppercase flex items-center gap-1.5 transition-all shadow-xs active:scale-95"
                        >
                          <Printer size={14} /> Ver / Imprimir Vale
                        </button>
                      </div>
                    </div>
                  ))}
      ...
      {/* MODAL DE IMPRESIÓN DE VALE DE ENTRADA FJ */}
      {selectedValeToPrint && (
