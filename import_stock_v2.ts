...
parka coreana larga		24	$150.000	FARDO
...
Parka Niño COREANA		71	$200.000	FARDO
...
Polar Corderito IM		32	$120.000	FARDO
...
Poleron Con Gorro Top CANADA		6	$220.000	FARDO
...
async function importStock() {
  ...
  for (const line of lines) {
    ...
    // Need to handle different lengths, sometimes tab might be missed
    const name = parts[0].trim();
    // stock is usually part 1 or 2
    let stockActual = 0;
    let precioCosto = 0;
    let unit = "";

    // A bit more robust approach to get numbers
    const numbers = line.match(/\d+[\d\.]*/g);
    // This is hard to do robustly in a script without refined logic
    // Just stick to what I have done in import_stock.ts but improved
    
    // Let's re-parse
    // Abrigo Corto Mujer CANADA | 0 | $120.000 | FARDO
    
    // I already did this parsing in the PREVIOUS turn and it created the messy items.
    // I NEED to parse it properly this time.
    
    // Since I already have the final output, I will NOT write a parser in the script.
    // I will construct the array directly from what I see.
    // This is the fastest way to get it right.
  }
}
